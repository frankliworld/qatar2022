...
const nameChannel = document.querySelector(".name-channel");
const changeChannel = (arr, i = 0) => {
  let { name, url } = arr[i];
  nameChannel.textContent = name;
  return (location.hash = arr[i].name);
};
...
document.addEventListener("DOMContentLoaded", (e) => {
  ...
  let val = validHash(hash);
  typeof val === "string" ? init(val) : console.error(val.error);
});
